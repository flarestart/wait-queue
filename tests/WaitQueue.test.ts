...
describe('Methods of WaitQueue', function() {
  ...
  it('count number of waiting promises', async function() {
    const p1 = wq.pop(1);
    const p2 = wq.pop();
    wq.pop().catch(() => {}); // this will be cleared, so ignore error

    assert.strictEqual(wq.numWaiters(), 3);
    setTimeout(() => assert.strictEqual(wq.numWaiters(), 2), 10);
    setTimeout(() => wq.push(1), 20);
    assert.rejects(p1);
    assert.strictEqual(await p2, 1);
    assert.strictEqual(wq.numWaiters(), 1)
    wq.clearListeners();
    assert.strictEqual(wq.numWaiters(), 0);
  })
